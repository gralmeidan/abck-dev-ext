/// REF: https://github.com/zedd3v/abck-dev
...
function recursiveSplitByDivider(
  ...
  if (detailed && children && children.length > 0) {
    children.forEach(child => {
      ...
      const childParsedSensor = recursiveSplitByDivider(
        value,
        detailed,
        child,
        seperator
...
export function ParseNewSensor(
  sensor: string,
  detailed: boolean
): ParsedSensor | null {
  try {
    ...
    return (
      // console.error(e),
      // alert(
      //   'An error occured, recheck your sensor. (More info in the console)'
      // ),
      null
    );
  }
...
export function parseSensor(
  ...
  const parsed = ParseNewSensor(sensor.replace(/^(\d\D)0\D/, '$1'), detailed);
